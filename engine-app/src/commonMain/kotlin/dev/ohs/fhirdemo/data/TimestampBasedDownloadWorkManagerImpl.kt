...
class TimestampBasedDownloadWorkManagerImpl(private val dataStore: DemoDataStore) :
    ...
    override suspend fun processResponse(response: Resource): Collection<Resource> {
        ...
        if (response is Bundle) {
            val nextUrl = response.link.firstOrNull { component -> component.relation.value == "next" }?.url?.value
            if (nextUrl != null) {
                urls.add(nextUrl)
            }
        }
        ...
        resources
            ...
            .forEach { map ->
                dataStore.saveLastUpdatedTimestamp(
                    ...
                    map.value.mapNotNull { it.meta?.lastUpdated?.value as? FhirDateTime.DateTime }.maxByOrNull { it.dateTime.toInstant(it.utcOffset).epochSeconds }?.toString() ?: "",
                )
            }
    }
}
