        ...
        return when (workInfoState) {
            ...
            WorkInfo.State.RUNNING -> {
                return when (syncJobStatusFromWorkManager) {
                    ...
                    is SyncJobStatus.InProgress, -> CurrentSyncJobStatus.Running(
                        syncJobStatusFromWorkManager
                    )
                    is SyncJobStatus.Succeeded -> CurrentSyncJobStatus.Succeeded(
                        syncJobStatusFromWorkManager.timestamp
                    )
                    is SyncJobStatus.Failed -> CurrentSyncJobStatus.Failed(
                        syncJobStatusFromWorkManager.timestamp
                    )
