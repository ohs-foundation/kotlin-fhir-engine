...
import dev.ohs.fhir.sync.MAX_RETRIES_ALLOWED
import dev.ohs.fhir.sync.NetworkType
...
import dev.ohs.fhir.sync.SyncConstraints
import dev.ohs.fhir.sync.SyncJobStatus
import dev.ohs.fhir.sync.SyncScheduler
import dev.ohs.fhir.sync.UNIQUE_WORK_NAME
...
/**
 * Android implementation of [dev.ohs.fhir.sync.SyncScheduler] that uses [WorkManager] to schedule sync jobs.
 *
 * @param context The application context.
 * @param workerClass The class of the [FhirSyncWorker] to be used for sync jobs.
 * @param dataStore The [dev.ohs.fhir.sync.FhirDataStore] instance for persisting sync state and metadata.
 */
@PublishedApi
internal class AndroidSyncScheduler(
   ...
   * @return a [Flow] of [dev.ohs.fhir.sync.CurrentSyncJobStatus]
   */
   ...
   * @return a [Flow] of [dev.ohs.fhir.sync.PeriodicSyncJobStatus]
   */
   ...
   * @return A flow emitting pairs of [WorkInfo.State] and [dev.ohs.fhir.sync.SyncJobStatus].
   */
...
private fun dev.ohs.fhir.sync.BackoffPolicy.toWorkManagerBackoffPolicy() =
  when (this) {
    dev.ohs.fhir.sync.BackoffPolicy.EXPONENTIAL -> BackoffPolicy.EXPONENTIAL
