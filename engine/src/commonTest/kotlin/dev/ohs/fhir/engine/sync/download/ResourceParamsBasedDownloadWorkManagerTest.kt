...
class ResourceParamsBasedDownloadWorkManagerTest {
  ...
  @Test
  fun getNextRequestUrl_shouldReturnNextResourceUrls() = runTest {
    val downloadManager =
    ...
    urlsToDownload shouldContainOnly
      listOf(
  ...
  @Test
  fun getNextRequestUrl_shouldReturnResourceAndPageUrlsAsNextUrls() = runTest {
    val downloadManager =
      ResourceParamsBasedDownloadWorkManager(
  ...
  @Test
  fun get_summary_request_urls_should_return_resource_summary_urls() = runTest {
    val downloadManager =
      ResourceParamsBasedDownloadWorkManager(
        mapOf(
          ResourceType.Patient to mapOf("address-city" to "NAIROBI"),
    ...
    urls.map { it.key } shouldContainOnly
      listOf(ResourceType.Patient, ResourceType.Immunization, ResourceType.Observation)
    urls.map { it.value } shouldContainOnly
      listOf(
  ...
  @Test
  fun process_response_should_throw_exception_including_diagnostics_from_operation_outcome() =
    runTest {
      val downloadManager =
        ResourceParamsBasedDownloadWorkManager(
          emptyMap(),
  ...
  @Test
  fun process_response_should_return_empty_list_for_resource_that_is_not_a_bundle() = runTest {
    val downloadManager =
      ResourceParamsBasedDownloadWorkManager(
        emptyMap(),
        NoOpResourceParamsBasedDownloadWorkManagerContext,
  ...
  @Test
  fun process_response_should_return_empty_list_for_bundle_that_is_not_a_search_set() = runTest {
    val downloadManager =
      ResourceParamsBasedDownloadWorkManager(
        emptyMap(),
        NoOpResourceParamsBasedDownloadWorkManagerContext,
  ...
  @Test
  fun process_response_should_return_resources_for_bundle_search_set() = runTest {
    val downloadManager =
      ResourceParamsBasedDownloadWorkManager(
        emptyMap(),
        NoOpResourceParamsBasedDownloadWorkManagerContext,
    ...
    downloadManager.processResponse(response).map { it.id } shouldContainOnly
      listOf("Patient-Id-001", "Patient-Id-002")
  }

  @Test
  fun process_response_should_add_next_request() = runTest {
    val downloadManager =
      ResourceParamsBasedDownloadWorkManager(
        emptyMap(),
        NoOpResourceParamsBasedDownloadWorkManagerContext,
  ...
  @Test
  fun process_response_should_not_add_next_request_if_next_url_is_missing() = runTest {
    val downloadManager =
      ResourceParamsBasedDownloadWorkManager(
        emptyMap(),
        NoOpResourceParamsBasedDownloadWorkManagerContext,
