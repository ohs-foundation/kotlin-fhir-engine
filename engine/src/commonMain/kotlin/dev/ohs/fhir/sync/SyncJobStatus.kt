...
sealed class SyncJobStatus {
  ...
  @Serializable @SerialName("Started") class Started : SyncJobStatus()
  ...
  @Serializable @SerialName("Succeeded") class Succeeded : SyncJobStatus()

  /** Sync job failed. */
  @Serializable
  @SerialName("Failed")
  data class Failed(
      @Transient val exceptions: List<ResourceSyncException> = emptyList(),
  ) : SyncJobStatus()
}
