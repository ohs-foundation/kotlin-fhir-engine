...
import dev.ohs.fhir.sync.upload.request.BundleUploadRequestMapping
  ...
  private fun handleSuccessfulUploadResponse(
    ...
    val responsesList =
        when (mappedUploadRequest) {
            is UrlUploadRequestMapping if response is DomainResource ->
                listOf(ResourceUploadResponseMapping(mappedUploadRequest.localChanges, response))

            is BundleUploadRequestMapping if response is Bundle &&
                    response.type.value == Bundle.BundleType.Transaction_Response ->
                handleBundleUploadResponse(mappedUploadRequest, response)

            else -> throw IllegalStateException(
                "Unknown mapping for request and response. Request Type: ${mappedUploadRequest::class}, Response Type: ${response::class.simpleName}",
            )
        }
